...
const showVariantPicker = (product) => {
  ...
  modal.innerHTML = `
    <div class="modal-modern" style="max-width: 400px;">
      <div class="modal-header-modern">
        ...
        <button class="modal-close-modern" id="variantPickerClose">
  ...
  // FIX: Use event listener instead of inline onclick (CSP safe)
  modal.querySelector('#variantPickerClose').addEventListener('click', () => modal.remove());

  const variantGrid = modal.querySelector('#variantGrid');
  variantGrid.innerHTML = product.variants.map((variant, i) => `
    <button class="variant-option" data-variant-index="${i}">
  ...
  variantGrid.querySelectorAll('.variant-option').forEach(btn => {
    btn.addEventListener('click', () => {
      // FIX: Use index instead of JSON.parse on data attribute (avoids parse errors)
...
// FIX: Correct CSS class name, cartCount badge update, variant_id null handling
const renderCart = () => {
  const container = $('#cartItems');
  if (!container) return;

  // FIX: Update cartCount badge on every render
  const countEl = $('#cartCount');
  const totalItems = state.cart.reduce((s, i) => s + i.qty, 0);
  if (countEl) countEl.textContent = totalItems;

  if (state.cart.length === 0) {
    // FIX: Correct class 'cart-empty-modern' (original had 'cart-empty' which has no CSS)
    container.innerHTML = `<div class="cart-empty-modern">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none">
        <path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z" stroke="#D4D4D8" stroke-width="1.5"/>
        <line x1="3" y1="6" x2="21" y2="6" stroke="#D4D4D8" stroke-width="1.5"/>
      </svg>
    ...
    <div class="cart-item">
      <div class="cart-item-top">
        ...
        <button class="cart-item-remove"
          data-remove-pid="${item.product_id}"
          data-remove-vid="${item.variant_id != null ? item.variant_id : ''}">
      ...
      <div class="cart-item-bottom">
        <div class="qty-control">
          <button class="qty-btn"
            data-qty-pid="${item.product_id}"
            data-qty-vid="${item.variant_id != null ? item.variant_id : ''}"
            data-d="-1">−</button>
...
const renderVariantsList = () => {
  ...
  if (currentVariants.length === 0) {
    variantsList.innerHTML = '<div class="empty-variants" style="padding: 1rem; text-align: center; color: var(--muted-foreground);">No size options added. Click "Add Size" to get started.</div>';
    return;
  }

  variantsList.innerHTML = currentVariants.map((variant, index) => `
    <div class="variant-item" data-index="${index}">
      <select class="variant-type" data-index="${index}">
        ...
        <option value="3 Scoops" ${variant.size_label === '3 Scoops' ? 'selected' : ''}>3 Scoops</option>
      ...
      <input type="number" class="variant-price-input" data-index="${index}"
  ...
  select.addEventListener('change', (e) => {
    ...
    currentVariants[idx].size_label = label;

    // ✅ SCOOP (fixed + dynamic)
    if (label.includes('Scoop')) {
      currentVariants[idx].size_type = 'scoop';
      currentVariants[idx].size_value = label.split(' ')[0]; // "2 Scoops" → "2"
    ...
    // ✅ CONTAINER
      ...
      currentVariants[idx].size_value =
        label === 'Half Ltr' ? 'half_ltr' : 'ltr';

    // ✅ FALLBACK (safety)
    } else {
      currentVariants[idx].size_type = 'custom';
      currentVariants[idx].size_value = label.toLowerCase().replace(/\s+/g, '_');
    }
  });
});

  // FIX: Use 'input' event for real-time price updates
...
const initVariantManager = () => {
  ...
  hasVariantsCheckbox.addEventListener('change', (e) => {
    if (e.target.checked) {
      variantManager.classList.remove('hidden');
      if (currentVariants.length === 0) loadPreset('size');
    } else {
  ...
  addVariantBtn?.addEventListener('click', () => {
    currentVariants.push({
      size_type: 'size',
      size_value: 'small',
      size_label: 'Small', // FIX: default to valid select option
      price: 0
    });
    renderVariantsList();
  });
...
// FIX: Removed stray "eures" text — was breaking every table row render
const renderProductsTable = products => {
  ...
  tbody.innerHTML = products.map((p, i) => {
    ...
    const imgCell = imgSrc
      ? `<img src="${imgSrc}" class="table-product-img" alt="${escHtml(p.name)}" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'table-product-img table-product-img--empty\'><svg width=\'16\' height=\'16\' viewBox=\'0 0 24 24\' fill=\'none\'><rect x=\'3\' y=\'3\' width=\'18\' height=\'18\' rx=\'3\' stroke=\'#C7C7CC\' stroke-width=\'1.5\'/></svg></div>';" />`
      : `<div class="table-product-img table-product-img--empty"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="3" stroke="#C7C7CC" stroke-width="1.5"/><circle cx="8.5" cy="8.5" r="1.5" fill="#C7C7CC"/><path d="M3 14l5-5 4 4 3-3 6 6" stroke="#C7C7CC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></div>`;

    // FIX: Was "eures" (corrupted) — now correct <tr> tag
    return `<tr>
      ...
      <td><span class="badge-cat" style="background:color-mix(in srgb,${catColor} 15%,white);color:${catColor}">
        <span class="cat-dot" style="background:${catColor}"></span>${escHtml(catName)}
      </span></td>
      <td><strong>${priceDisplay}</strong></td>
      <td><span class="badge ${p.stock > 0 ? 'badge-green' : 'badge-zinc'}">${p.stock}</span></td>
      <td>
        <button class="btn-edit-ghost" data-edit="${p.id}">Edit</button>
        <button class="btn-danger-ghost" data-del="${p.id}">Delete</button>
      </td>
    </tr>`;
  }).join('');
...
const updateModalImagePreview = (src) => {
  ...
  if (src) {
    ...
    preview.innerHTML = `<div class="image-preview-empty"><svg width="32" height="32" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="18" height="18" rx="3" stroke="#C7C7CC" stroke-width="1.5"/><circle cx="8.5" cy="8.5" r="1.5" fill="#C7C7CC"/><path d="M3 14l5-5 4 4 3-3 6 6" stroke="#C7C7CC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Click to add photo</span></div>`;
...
const openEditProduct = async id => {
  ...
  // FIX: Deep copy variants to avoid mutating cache
  currentVariants = (p.variants || []).map(v => ({ ...v }));
  if (currentVariants.length > 0) {
    $('#hasVariants').checked = true;
    $('#variantManager').classList.remove('hidden');
    renderVariantsList();
...
// FIX: Reliable state cleanup, try/catch for image deletion
const closeProductModal = async () => {
  try {
    if (state.pendingImageFile) {
      await window.api.image.delete(state.pendingImageFile);
    }
  } catch (e) {
    console.warn('Could not delete pending image:', e);
  }
...
const saveProduct = async () => {
  ...
  if (hasVariants) {
    const invalidVariants = currentVariants.filter(v => !v.price || v.price <= 0);
  ...
  try {
    const productData = {
      name,
      // FIX: Don't send garbage price when hasVariants; use 0
      price: hasVariants ? 0 : price,
      stock, category, category_id, image: imageToSave,
      hasVariants,
...
const loadHistory = async () => {
  ...
  try {
    ...
    if (invoices.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" class="table-loading">No invoices found</td></tr>';
      return;
    }
    tbody.innerHTML = invoices.map(inv => {
      ...
      return `<tr>
        <td><span class="badge badge-zinc">${escHtml(inv.invoice_no)}</span></td>
        <td>${escHtml(inv.customer || 'Walk-in Customer')}</td>
        <td><span class="badge badge-zinc">${inv.item_count} item${inv.item_count !== 1 ? 's' : ''}</span></td>
        <td><strong>${fmt(inv.total)}</strong></td>
        <td style="color:var(--muted-foreground);font-size:12.5px">${ds} ${ts}</td>
        <td><button class="btn-edit-ghost" data-reprint="${inv.id}">View</button></td>
      </tr>`;
    }).join('');
    $$('[data-reprint]', tbody).forEach(btn => btn.addEventListener('click', () => reprintInvoice(Number(btn.dataset.reprint))));
  } catch (err) {
    tbody.innerHTML = '<tr><td colspan="6" class="table-loading">Error loading invoices</td></tr>';
    console.error('loadHistory error:', err);
  }
};

// FIX: discountPct / taxPct are now numbers not strings (no .toFixed on them)
const reprintInvoice = async id => {
  try {
    ...
    const discountPct = inv.subtotal > 0 ? (inv.discount / inv.subtotal) * 100 : 0;
    const taxPct = base > 0 ? (inv.tax / base) * 100 : 0;
    ...
    console.error('reprintInvoice error:', err);
  }
};

// ── CLEAR HISTORY FEATURE ─────────────────────────────────────
let clearHistoryModalEl = null;
let clearHistoryInited = false;

const showClearHistoryModal = () => {
  if (!clearHistoryModalEl) {
    clearHistoryModalEl = $('#clearHistoryModal');
    if (!clearHistoryModalEl) return;
  }

  // FIX: Attach listeners only once — prevents duplicate handler stacking
  if (!clearHistoryInited) {
    clearHistoryInited = true;
    ...
    $$('[data-days]').forEach(btn => {
      btn.addEventListener('click', () => clearOlderThan(parseInt(btn.dataset.days, 10)));
    });
    // FIX: Backdrop click closes the modal
    clearHistoryModalEl.addEventListener('click', (e) => {
      if (e.target === clearHistoryModalEl) closeClearHistoryModal();
    });
  }

  clearHistoryModalEl.classList.remove('hidden');
};

const closeClearHistoryModal = () => {
  if (clearHistoryModalEl) clearHistoryModalEl.classList.add('hidden');
};
...
const confirmClearAll = async () => {
  ...
  try {
    ...
    console.error('confirmClearAll:', error);
  }
};
...
const clearOlderThan = async (days) => {
  ...
  try {
    ...
    showToast('Error clearing history');
    console.error('clearOlderThan:', error);
  }
};
...
const clearByDateRange = async () => {
  ...
  try {
    ...
    showToast('Error clearing history');
    console.error('clearByDateRange:', error);
  }
};
...
const loadDashboard = async () => {
  try {
    ...
    // FIX: Explicit period:'all' to ensure no accidental filtering
    const recent = await window.api.invoices.getAll({ period: 'all' });
    const tbody = $('#dashRecentInvoices');
    if (!tbody) return;
    if (recent.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" class="table-loading">No invoices yet</td></tr>';
      return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!window.api) {
    ...
    showToast('Error: API not available');
    return;
  }
  initNav();
  await initBilling();
